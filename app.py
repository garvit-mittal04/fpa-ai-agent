...
if st.session_state.get("analysis_ran", False):
    ...
    chart_variance_df = variance_df.copy()
    if selected_period != "All Periods" and "period" in chart_variance_df.columns:
        chart_variance_df = chart_variance_df[
            chart_variance_df["period"].astype(str) == str(selected_period)
        ]

    top10 = chart_variance_df.head(10).copy()
    ...
    if not top10.empty:
        ...
        fig_waterfall = go.Figure(
            go.Bar(
                x=top10["line_item"].astype(str) + "<br><span style='font-size:10px'>(" + top10["period"].astype(str) + ")</span>",
        ...
        st.info("No variance records available for the selected period.")

    st.divider()

    trend_title = (
        "### 📈 Rolling 3-Month Trend by Department"
        if selected_period == "All Periods"
        else f"### 📈 Trend View for {selected_period}"
    )
    st.markdown(trend_title)

    chart_trends_df = trends_df.copy()
    if selected_period != "All Periods" and "period" in chart_trends_df.columns:
        chart_trends_df = chart_trends_df[
            chart_trends_df["period"].astype(str) == str(selected_period)
        ]

    if not chart_trends_df.empty:
        palette = [
        ...
        fig_trend = go.Figure()
        for i, dept in enumerate(chart_trends_df["department"].unique()):
            d = chart_trends_df[chart_trends_df["department"] == dept]
            fig_trend.add_trace(
                go.Scatter(
                    x=d["period"],
                    y=d["rolling_3m_avg"],
        ...
        st.info("No trend records available for the selected period.")
